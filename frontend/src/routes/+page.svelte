<script lang="ts">
  ...
  async function exportCsv() {
    const active = cleanRows();
    if (!active.length) { alert('Dışa aktarılacak satır yok.'); return; }
    const esc = (s: string) => /[;"\n\r]/.test(s) ? '"' + s.replace(/"/g, '""') + '"' : s;
    const header = 'Eski Cari Kodu;Yeni Cari Kodu;Eski Kart Silinsin;Durum';
    const body = active.map((r) => [r.eski, r.yeni, r.sil ? '1' : '0', r.message ?? ''].map(esc).join(';'));
    const csv = '﻿' + [header, ...body].join('\r\n') + '\r\n'; // BOM: Excel Türkçe
    try {
      const path = await save({ defaultPath: 'cari-aktarim.csv', filters: [{ name: 'CSV', extensions: ['csv'] }] });
      if (!path) return;
