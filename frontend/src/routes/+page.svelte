<script lang="ts">
  ...
  let connectionOk = $state(false), backupOk = $state(false), running = $state(false), info = $state('Bağlantı test edilmedi');
  ...
  let panelHeights = $state([500, 320]);
  ...
  onMount(() => {
    ...
    return () => { unlistenRow?.(); unlistenLog?.(); unlistenProgress?.(); };
  });
  ...
  async function update() { try { const u = await check(); if (!u) return log('Uygulama güncel.'); if (confirm(`v${u.version} indirilsin ve kurulsun mu?`)) { log('Güncelleme indiriliyor…'); await u.downloadAndInstall(); } } catch (e) { log(`Güncelleme denetimi hatası: ${e}`); } }
...
<main>
  <header><div><h1>Cari Kartı Aktarma <small>098492</small></h1><p>Mikro V17 · Trigger Yönetimli</p></div><button class="secondary" onclick={update}>Güncelleme Denetle</button></header>
  ...
  <footer><button class="primary" onclick={testConnection} disabled={running}>Bağlantıyı Test Et</button><button class="danger" onclick={backup} disabled={!connectionOk || running}>Önce Yedek Al</button><button onclick={update} disabled={running}>Güncelleme Denetle</button><button onclick={triggerStatus} disabled={running}>Trigger Durumu</button><button class="outline" onclick={enableTriggers} disabled={running}>Trigger’ı Geri Aç</button><span>{info}</span></footer></section>
